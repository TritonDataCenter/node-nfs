...
SetAttrReply.prototype.set_wcc_data = function set_wcc_data() {
    this.wcc_data = wcc_data.create();
    return (this.wcc_data);
};
...
SetAttrReply.prototype._transform = function _transform(chunk, enc, cb) {
    if (this.incoming) {
        ...
        this.wcc_data = wcc_data.parse(xdr);
    } else {
        this.push(chunk);
    }
...
SetAttrReply.prototype.writeHead = function writeHead() {
    ...
    len += wcc_data.XDR_EMPTY_SIZE;

    // XXX once we support non-empty wcc_data, calculate the extra len

    var xdr = this._serialize(len);

    xdr.writeInt(this.status);
    wcc_data.serialize(xdr, this.wcc_data);
...
SetAttrReply.prototype.toString = function toString() {
    var fmt = '[object SetAttrReply <xid=%d, status=%d, wcc_data=%j>]';
    return (util.format(fmt, this.xid, this.status, this.wcc_data));
};
