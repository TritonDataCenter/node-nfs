...
NfsClient.prototype.readlink = function readlink(symlink, cb) {
    assert.string(symlink, 'symlink');
    assert.func(cb, 'callback');
...
NfsClient.prototype.write = function write(file, offset, count, stable, cb) {
    assert.string(file, 'object');
    assert.number(offset, 'offset');
    assert.number(count, 'count');
